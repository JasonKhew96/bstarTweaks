...
//    jmethodID get_declared_field;
    ...
    struct MemMap {
        ...
        noexcept : addr_(other.addr_), len_(other
        .len_) {
            other.addr_ = nullptr;
            other.len_ = 0;
        }
    ...
    struct [[gnu::packed]] ZipLocalFile {
        ...
        ZipLocalFile *next() {
            return from(
                    reinterpret_cast<uint8_t *>(this) + sizeof(ZipLocalFile) + file_name_length +
                    extra_length + compress_size);
        }
        ...
        MemMap uncompress() {
            if (compress == 0x8) {
                ...
                if (!out.ok()) {
                    PLOGE("failed to mmap for uncompression");
                    return {};
                }
                int err;
    ...
    jclass LoadClass(JNIEnv *env, jobject class_loader, std::string_view descriptor) {
        if (descriptor.empty())
            return nullptr;
        if (descriptor[0] != 'L')
            return nullptr;
        ...
        auto res = (jclass) env->CallObjectMethod(class_loader, load_class_method, java_name, false);
        return res;
    }
...
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_findMethodUsingString(JNIEnv *env, jobject thiz,
                                                                       jstring str,
                                                                       jboolean match_prefix,
                                                                       jlong return_type,
                                                                       jshort parameter_count,
                                                                       jstring parameter_shorty,
                                                                       jlong declaring_class,
                                                                       jlongArray parameter_types,
                                                                       jlongArray contains_parameter_types,
    ...
    auto parameter_shorty_ = parameter_shorty ? env->GetStringUTFChars(parameter_shorty, nullptr)
                                              : nullptr;
    ...
    if (dex_priority) {
        ...
        dex_priority_.assign(dex_priority_elements,
                             dex_priority_elements + env->GetArrayLength(dex_priority));
    }

    std::vector <size_t> parameter_types_;
    jlong *parameter_types_elements = nullptr;
    if (parameter_types) {
        parameter_types_elements = env->GetLongArrayElements(parameter_types, nullptr);
        parameter_types_.assign(parameter_types_elements,
                                parameter_types_elements + env->GetArrayLength(parameter_types));
    }

    std::vector <size_t> contains_parameter_types_;
    jlong *contains_parameter_types_elements = nullptr;
    if (contains_parameter_types) {
        contains_parameter_types_elements = env->GetLongArrayElements(contains_parameter_types,
                                                                      nullptr);
        contains_parameter_types_.assign(contains_parameter_types_elements,
                                         contains_parameter_types_elements +
                                         env->GetArrayLength(contains_parameter_types));
    }

    auto out = helper->FindMethodUsingString(str_, match_prefix, return_type, parameter_count,
                                             parameter_shorty_ ? parameter_shorty_ : "",
                                             declaring_class, parameter_types_,
                                             contains_parameter_types_, dex_priority_, find_first);
    ...
    if (dex_priority_elements)
        env->ReleaseIntArrayElements(dex_priority, dex_priority_elements, JNI_ABORT);
    if (parameter_types_elements)
        env->ReleaseLongArrayElements(parameter_types, parameter_types_elements, JNI_ABORT);
    if (contains_parameter_types_elements)
        env->ReleaseLongArrayElements(contains_parameter_types, contains_parameter_types_elements,
                                      JNI_ABORT);
...
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_load(JNIEnv *env, jobject thiz, jobject class_loader) {
    if (!class_loader)
        return 0;
    ...
    for (auto i = 0, len = env->GetArrayLength(elements); i < len; ++i) {
        ...
        if (!java_dex_file)
            continue;
        auto cookie = (jlongArray)
        env->GetObjectField(java_dex_file, cookie_field);
        if (!cookie)
            continue;
            ...
            if (zip_file) {
                for (int idx = 1;; ++idx) {
                    ...
                    if (entry) {
                        ...
                        if (uncompress.ok()) {
                            LOGD("uncompressed %.*s", static_cast<int>(entry->file_name().size()),
    ...
    if (images.empty())
        return 0;
    auto res = reinterpret_cast<jlong>(new Handler(std::make_unique<DexHelper>(images),
                                                   std::move(maps)));
...
extern "C" JNIEXPORT jlongArray
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_findMethodInvoking(JNIEnv *env, jobject thiz,
                                                                    jlong method_index,
                                                                    jlong return_type,
                                                                    jshort parameter_count,
                                                                    jstring parameter_shorty,
    ...
    auto out = helper->FindMethodInvoking(method_index, return_type, parameter_count,
                                          parameter_shorty_ ? parameter_shorty_ : "",
                                          declaring_class, parameter_types_,
                                          contains_parameter_types_, dex_priority_, find_first);
...
extern "C" JNIEXPORT jlongArray
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_findMethodInvoked(JNIEnv *env, jobject thiz,
                                                                   jlong method_index,
                                                                   jlong return_type,
                                                                   jshort parameter_count,
                                                                   jstring parameter_shorty,
    ...
    auto out = helper->FindMethodInvoked(method_index, return_type, parameter_count,
                                         parameter_shorty_ ? parameter_shorty_ : "",
                                         declaring_class, parameter_types_,
                                         contains_parameter_types_, dex_priority_, find_first);
...
extern "C" JNIEXPORT jlongArray
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_findMethodSettingField(JNIEnv *env, jobject thiz,
                                                                        jlong field_index,
                                                                        jlong return_type,
                                                                        jshort parameter_count,
                                                                        jstring parameter_shorty,
    ...
    auto out = helper->FindMethodSettingField(field_index, return_type, parameter_count,
                                              parameter_shorty_ ? parameter_shorty_ : "",
                                              declaring_class, parameter_types_,
                                              contains_parameter_types_, dex_priority_, find_first);
...
extern "C" JNIEXPORT jlongArray
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_findMethodGettingField(JNIEnv *env, jobject thiz,
                                                                        jlong field_index,
                                                                        jlong return_type,
                                                                        jshort parameter_count,
                                                                        jstring parameter_shorty,
    ...
    auto out = helper->FindMethodGettingField(field_index, return_type, parameter_count,
                                              parameter_shorty_ ? parameter_shorty_ : "",
                                              declaring_class, parameter_types_,
                                              contains_parameter_types_, dex_priority_, find_first);
...
extern "C" JNIEXPORT jlongArray
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_findField(JNIEnv *env, jobject thiz, jlong type,
                                                           jintArray dex_priority,
                                                           jboolean find_first) {
    auto *handler = reinterpret_cast<Handler *>(env->GetLongField(thiz, token_field));
    if (!handler)
...
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_decodeMethodIndex(JNIEnv *env, jobject thiz,
                                                                   jlong method_index) {
    auto *handler = reinterpret_cast<Handler *>(env->GetLongField(thiz, token_field));
    if (!handler)
        return env->NewLongArray(0);
...
extern "C" JNIEXPORT jobject
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_decodeFieldIndex(JNIEnv *env, jobject thiz,
                                                                  jlong field_index) {
    auto *handler = reinterpret_cast<Handler *>(env->GetLongField(thiz, token_field));
    if (!handler)
        return env->NewLongArray(0);
...
extern "C" JNIEXPORT jlong
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_encodeClassIndex(JNIEnv *env, jobject thiz, jclass clazz) {
    auto *handler = reinterpret_cast<Handler *>(env->GetLongField(thiz, token_field));
    if (!handler)
        return -1;
    auto &[helper, _] = *handler;
...
extern "C" JNIEXPORT jlong
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_encodeFieldIndex(JNIEnv *env, jobject thiz,
                                                                  jobject field) {
    auto *handler = reinterpret_cast<Handler *>(env->GetLongField(thiz, token_field));
    if (!handler)
        return -1;
...
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_encodeMethodIndex(JNIEnv *env, jobject thiz,
                                                                   jobject method) {
    auto *handler = reinterpret_cast<Handler *>(env->GetLongField(thiz, token_field));
    if (!handler)
        return -1;
    ...
    auto clazz = (jclass) env->CallObjectMethod(method, get_declaring_class_method);
    auto name = env->GetStringUTFChars(java_name, nullptr);
    auto params = (jobjectArray) env->CallObjectMethod(method, get_parameters_method);
    ...
    auto res = helper->CreateMethodIndex(GetClassDescriptor(env, clazz), name, params_name);
    env->ReleaseStringUTFChars(java_name, name);
    return static_cast<jlong>(res);
}

extern "C" JNIEXPORT jclass

JNICALL Java_me_iacn_biliroaming_utils_DexHelper_decodeClassIndex(JNIEnv *env, jobject thiz,
                                                                  jlong class_index) {
    auto *handler = reinterpret_cast<Handler *>(env->GetLongField(thiz, token_field));
    if (!handler)
        return nullptr;
...
Java_me_iacn_biliroaming_utils_DexHelper_close(JNIEnv
*env,
jobject thiz
) {
auto *handler = reinterpret_cast<Handler *>(env->GetLongField(thiz, token_field));
env->
SetLongField(thiz, token_field, jlong(0)
);
delete
handler;
}
...
JNICALL JNI_OnLoad(JavaVM *vm, void *) {
    ...
    load_class_method = env->GetMethodID(class_loader, "loadClass",
                                         "(Ljava/lang/String;Z)Ljava/lang/Class;");
    auto member = env->FindClass("java/lang/reflect/Member");
    get_declaring_class_method = env->GetMethodID(member, "getDeclaringClass",
                                                  "()Ljava/lang/Class;");
    ...
    get_parameters_method = env->GetMethodID(executable, "getParameterTypes",
                                             "()[Ljava/lang/Class;");
    auto clazz = env->FindClass("java/lang/Class");
    get_class_name_method = env->GetMethodID(clazz, "getName", "()Ljava/lang/String;");
    auto dex_class_loader = env->FindClass("dalvik/system/BaseDexClassLoader");
    path_list_field = env->GetFieldID(dex_class_loader, "pathList", "Ldalvik/system/DexPathList;");
...
JNICALL Java_me_iacn_biliroaming_utils_DexHelper_createFullCache(JNIEnv *env, jobject thiz) {
auto *handler = reinterpret_cast<Handler *>(env->GetLongField(thiz, token_field));
if (!handler)
return;
auto &[helper, _] = *handler;
